...
Now it is the turn of vLLM, the other clear contendar for a simple library to run LLMs locally. The installation process is easy, everything looks easy after [the experience with TensorRT-LLM](post_url 2025-11-24-tensorrt-llm.md).

I followed the instructions in the vLLM documentaion, creating a virtual environment to avoid messing with existing Pytorch and CUDA libraries:
...
I compiled the library from the source in two different ways, just to see how long they would take. First, using precompiled librarios:
...
And then, I compiled everythiing from the source:
```bash
git clone https://github.com/vllm-project/vllm.git
cd vllm
uv pip install -e .
