...
# Running the models
As with Qwen3-Coder, I'll use the 4bits quantization, the only one that can reasonably run on my PC:
```bash
 hf download /
...
I asked ChatGPT for a prompt for this test. He sugessted this problem:
...
It takes two and a half minutes to produce a solution. Less than 6,000 tokes, at 40 tokens/s
...
The answer includes a table showing how the code implements each requirement, with special emphasys on how it avoids race conditions (_spoiler_: it doesn't). It finishes with instructions to run the code and a list of possible improvements.

## Qwen3.5-27B-Claude-4.6-Opus-Reasoning-Distilled-v2

This is a thinking model, so the we get some "thinking time" before getting the solution. Also, unlike the previous model, this one is not based on a MoE architecture, so all layers are always active, and we can't move them from RAM to VRAM. Some of the layers will RAM using CPU instead of GPU, decreasing the performance.
...
The style is similar to Qwen3-Coder, but it looks less elegant: more loops, a huge sucession of if-elif sentences for the events, comments of the functions but without any additiona information,...
...
So it doesn't look too bad, until you look at the details on most basic feature requested.
For Qwen3-Coder:
...
Now, that sounds are more reasonable score if, indeed, the basic features don't work (altought Claude thinks that the Opus version is almost ok, mmm, suspicious).
